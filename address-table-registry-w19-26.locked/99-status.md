...
| M8_LIVE_VALIDATION | partial | gateway#565+#566 deploy | post-merge live (33+ min uptime): source-selection active_probe_passed source=0x7F companion=0x84; existing devices stable; wire-up + src-insertion semantics verified by unit tests; in 715 observed bus messages (529 active gateway + 186 passive), zero frames from src=0xF1 — only 0x10 broadcasts (no ACK, not insertion-eligible) + 0x10 scan_timeouts (no ACK). P1 live confirmation requires organic NETX3 emission, which is sparser than expected in this deployment. Operator can trigger via KNX setting change or wait for next periodic enrichment cycle. P2/P3 N/A by default flag |
