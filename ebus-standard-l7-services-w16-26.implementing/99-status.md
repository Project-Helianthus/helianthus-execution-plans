...
- Current milestone target: `M6a_transport_matrix_artifact` (live-bus matrix artifact — depends on operator BENCH-REPLACE bench data)
...
- **Follow-up task**: gateway-side `forward_compat_synthetic_v1_1.golden.json` should add unknown-active.transport case per M4D §10 conformance addition (separate producer-side task)
...
- **M6a_transport_matrix_artifact** — live-bus matrix artifact în helianthus-ebusgateway. All predecessors merged (M4B + M4D + M5 + M5b). Still depends on operator BENCH-REPLACE bench data from BASV2 live bus; M6a IMPL can land scaffolding now + freeze bench numbers as follow-up.
- **Follow-up producer task**: gateway-side `forward_compat_synthetic_v1_1.golden.json` should add unknown-active.transport case per M4D §10 conformance addition — small one-commit task în ebusgateway.
...
1. **M6a_transport_matrix_artifact** dispatch în helianthus-ebusgateway — live-bus conformance matrix: offline conformance vectors + Vaillant regression transcripts + NM wire regression + 07 FF cadence floor + rollback criteria per repo. Scaffolding can land now; bench numbers finalized after BENCH-REPLACE operator data available.
2. **Follow-up producer task** în helianthus-ebusgateway: extend `forward_compat_synthetic_v1_1.golden.json` with unknown-active.transport case (pairs unknown `active.transport` with known non-`none` `active.scope`) per M4D §10 conformance requirement.
3. **BENCH-REPLACE** operator follow-up (manual, separate): run timing harness pe BASV2 live bus, commit measured `responderAckBudget` as follow-up în ebusgo + ebusgateway per decision §7.1(1).
4. Post-M6a: **M6b** (docs publication + NM plan `.maintenance` transition + issue-map reconciliation) — sealed at plan close.
