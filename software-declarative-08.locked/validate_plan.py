...
def validate_plan(plan_dir: Path) -> dict[str, int]:
    ...
    expected = ["INT-18", "INT-22", "INT-23", "INT-24"]
    ...
    require(packages[1].get("depends_on") == ["INT-18"], "INT-22 must follow INT-18")
    require(packages[2].get("depends_on") == ["INT-22"], "INT-23 must follow INT-22")
    require(packages[3].get("depends_on") == ["INT-23"], "INT-24 must follow INT-23")
