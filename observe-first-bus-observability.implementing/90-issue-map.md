...
| `ISSUE-GW-18I` | `helianthus-ebusgateway` | Fix passive proxy-session contract on proxy-single ENS | active | issue `Project-Helianthus/helianthus-ebusgateway#366`, stacked PR `#367` from `issue/366-p03-passive-proxy-session-contract`; latest head `a7a46f609ede42ac3bf5ac3d461ecc656331e3ec`, rerun artifact `results-matrix-ha/20260311T200705Z-issue367-p03-rerun` failed, Codex review is clean on current head, and `#367` remains the active blocked lane not ready to fold upward into parent PR `#354` |
