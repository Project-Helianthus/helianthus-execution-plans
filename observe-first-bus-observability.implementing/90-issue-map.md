...
| `ISSUE-GW-18I` | `helianthus-ebusgateway` | Fix passive proxy-session contract on proxy-single ENS | active | issue `Project-Helianthus/helianthus-ebusgateway#366`; gateway harness PR `#375` was clean and folded upward into parent gateway lane `#373` as squash commit `97da9f9422a17951a94b1854783ce8f86be9a8da`, so remaining ownership now routes to proxy issue `#80` / PR `#81` on stream shape, with next proxy fix `request-window observer replay` for owner shorthand traffic |
