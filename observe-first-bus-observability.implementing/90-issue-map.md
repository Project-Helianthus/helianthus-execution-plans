...
| `ISSUE-GW-15` | `helianthus-ebusgateway` | Proof gate and default-flip preparation | active | issue `Project-Helianthus/helianthus-ebusgateway#400`, PR `#401` merge commit `5409c216654500a7b822ab620eacf2be59ae9497`, PR `#402` (`GW-15: add P03 canary manifest and interval verifier`) merge commit `1596c9fc02a0f745996cc5c911b9b1fec2c7c22d`, PR `#403` (`GW-15: add P03 canary proof verdict gate`) merge commit `a4cb5fb82b3adea42c45912e4c8f0a2d3e2db5bb`, and PR `#404` (`GW-15: add P03 proof window gate`) merge commit `977503bdab9a7bf6cb94b1b4a8243c86ed45e8b5`; `#401` closed the first bounded `P03` proof-plumbing slice, `#402` closed the second bounded slice (fixed `P03` canary manifest, active direct-read canary verifier, interval verifier integration, plus review hardening on baseline seeding, read-only method allowlist, first interval artifact scheduling, and semantic phase ordering), `#403` closed the third bounded slice (machine-readable canary verdict thresholds plus a fail-closed proof-mode gate on top of the artifact-production slices), and `#404` closed the fourth bounded slice (elapsed proof-window gate so proof mode cannot succeed on the first green snapshot plus hard poll failure resets); the first bounded merged-main rerun from an HA-reachable environment is `results-matrix-ha/20260315T032703Z-gw15-proof-p03/index.json`, which records `P03=fail` on gateway `main@977503bdab9a7bf6cb94b1b4a8243c86ed45e8b5` plus proxy `main@a141fe056cbf6a3e212f850b501a893f922f41f6` after startup/proxy passed and proof artifacts were emitted, because start-phase canary direct reads all hit `BUS_COLLISION`, `canary_baseline.json` stayed empty, and `canary_verdict.json` failed with zero conclusive intervals; `ISSUE-GW-15` remains active for subsequent bounded proof slices |
