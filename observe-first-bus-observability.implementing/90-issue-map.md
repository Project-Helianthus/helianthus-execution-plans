...
| `ISSUE-GW-18I` | `helianthus-ebusgateway` | Fix passive proxy-session contract on proxy-single ENS | active | issue `Project-Helianthus/helianthus-ebusgateway#366`, stacked PR `#369` on the `GW-18I` proof chain; latest head `daaa1cb`, rerun artifact `results-matrix-ha/20260311T202655Z-issue369-p03-rerun` failed, and `#369` remains the active blocked lane not ready to fold upward into `#367` / parent PR `#354` |
