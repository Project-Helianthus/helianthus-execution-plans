...
| `ISSUE-GW-18I` | `helianthus-ebusgateway` | Fix passive proxy-session contract on proxy-single ENS | active | issue `Project-Helianthus/helianthus-ebusgateway#366`, stacked PR `#371` on the `GW-18I` proof chain; latest head `0aafb42`, rerun artifact `results-matrix-ha/20260311T210157Z-issue371-p03-rerun` failed after reaching `LIVE_READY` with all four devices present, and `#371` remains the active blocked lane not ready to fold upward into `#369` / `#367` / parent PR `#354` |
