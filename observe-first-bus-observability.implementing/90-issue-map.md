...
| `ISSUE-GW-18I` | `helianthus-ebusgateway` | Fix passive proxy-session contract on proxy-single ENS | active | issue `Project-Helianthus/helianthus-ebusgateway#366`, combined rerun used proxy PR `#81` head `e75af3a` together with gateway PR `#373` head `0b5292e`; artifact `results-matrix-ha/20260312T055859Z-proxy81-gateway373-p03-rerun` failed after reaching `LIVE_READY` with all four devices present, and proxy PR `#81` is not yet proven enough to fold / merge while gateway PR `#373` cannot close `P03` |
