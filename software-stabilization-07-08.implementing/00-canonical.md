...
- `SEMREG-BOOTSTRAP`, after ownership selection in `INT-04`, establishes public `helianthus-semreg` for code and `helianthus-docs-semantic` for reusable architecture/API documentation, with licenses, self-contained AGENTS, validation and import/documentation contracts. `.github` remains the accountable package owner. The documentation contract must be merged before dependent code is accepted. Bootstrap precedes `INT-05` implementation; north–south design can continue in parallel.
